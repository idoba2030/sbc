...
rl_generator_single <- function(N,cfg){  # N is the number of data points we are generating
  #lambda <- rgamma(n = 1, shape = 15, rate = 5) generate params
  population_locations    =c(qlogis(0.5),4) #population mean  
  population_scales       =c(1,1.5)        #population sd 
  
  
  #individual parameters 
  alpha          = plogis(population_locations[1]+population_scales[1]*rnorm(N))
  beta           =       (population_locations[2]+population_scales[2]*rnorm(N))
